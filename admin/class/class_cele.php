...
class Cele {	
	...
	$stmtTotal = $this->conn->prepare("SELECT COUNT(*) as count FROM " . $this->examTable . " es JOIN " . $this->courseTable . " ec ON es.course_id = ec.id WHERE ec.course = 'CELE'");
	$stmtTotal->execute();
	$allResult = $stmtTotal->get_result()->fetch_assoc();
	$allRecords = $allResult['count'];
   
	// Get the total number of records (after filtering)
	$stmtFilteredTotal = $this->conn->prepare("SELECT COUNT(*) as count FROM " . $this->examTable . " es JOIN " . $this->courseTable . " ec ON es.course_id = ec.id WHERE ec.course = 'CELE' " . $filterQuery);
	if ($filterQuery) {
