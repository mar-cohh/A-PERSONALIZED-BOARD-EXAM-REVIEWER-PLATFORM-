...
class Questions {	
    ...
    private $questionBank = 'question_banks';
		...
		if ($this->subject_id && $this->question_title && $this->answer_option) {
			...
			$this->answer_option = intval($this->answer_option); // Ensure it's an integer
				...
				$stmt = $this->conn->prepare("
					INSERT INTO " . $this->questionBank . " (`subject_id`, `question`, `answer`)
					VALUES (?, ?, ?)");
				
				$stmt->bind_param("iss", $this->subject_id, $this->question_title, $this->answer_option);
				...
				if ($stmt->execute()) {
					...
					return json_encode(["status" => "error", "message" => "Error Creating question: " . $stmt->error]);
				}
		}
