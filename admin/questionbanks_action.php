...
include_once 'class/class_questionbanks.php';
    ...
    if (!empty($_FILES['csv_file']['name'])) {
        ...
        if (($handle = fopen($filename, "r")) !== FALSE) {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                ...
                // Prepare to insert question and options
                $questions->subject_id = $subject_id; // Set subject_id
                $questions->question_title = $question_title;
                $questions->answer_option = $answer_option;
                $questions->option = array_combine(range(1, 4), $options);
                
                // Insert the question
                if (!$questions->insert()) {
                    echo "Error inserting question on row $row!\n";
                    $hasError = true; // Set error flag
                }
            }
            fclose($handle);

            // Only echo success message if no errors occurred
            if (!$hasError) {
