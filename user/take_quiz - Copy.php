...
$query_questions = "SELECT * FROM exam_question WHERE subject_id = ?";
...
// Shuffle questions and limit to 2
if (!isset($_SESSION[$subject_key])) {
    shuffle($questions);
    $_SESSION[$subject_key] = array_slice($questions, 0, $question_limit); // Limit to 2 questions
}
...
$query_options = "SELECT * FROM exam_option WHERE question_id = ?";
                                    ...
                                    <?php foreach ($options as $option): ?>
                                        <div class="funkyradio-success">
                                            <input type="radio" id="option<?php echo $option['id']; ?>"
                                                name="answers[<?php echo $current_question['id']; ?>]"
                                                value="<?php echo $option['option']; ?>"
                                                onclick="storeAnswer(<?php echo $current_question['id']; ?>, '<?php echo $option['option']; ?>')"
                                                <?php echo (isset($_SESSION['answers'][$current_question['id']]) && $_SESSION['answers'][$current_question['id']] == $option['title']) ? 'checked' : 'unchecked'; ?>>
 ...
 <script>
    ...
    document.getElementById('qform').addEventListener('submit', function (e) {
        ...
        for (let questionId in storedAnswers) {
            ...
            hiddenInput.value = storedAnswers[questionId];
